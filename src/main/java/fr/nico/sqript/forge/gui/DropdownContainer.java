...
public class DropdownContainer extends Container {
			...
			if(extended){
				...
				if (style.drawBackground) {
					...
					if (draw_type == DRAW_IMAGE) {
						...
						drawTexturedModalRect((int) this.x, (int) this.y, (int) 0, 0, (int) display.width,
								(int) display.height);
					}
				}
				...
				for (int i = 0; i < content.size(); i++) {
					...
					if (layout == EnumLayout.NONE) {
						...
						if(!displayFromRight){
							...
							w.y = this.buttonDimensions.height+this.y + w.style.bordersize + w.style.margin_top + mtop + this.style.padding_top;
	...
	private void drawTop(int mouseX,int mouseY){
		...
		if(label_display_mode==LABEL_DRAW_TEXT){
			...
			Minecraft.getMinecraft().getTextureManager().bindTexture(imageSource);
			Render.drawTexturedRect((int) this.x+display.x, (int) this.y+display.y,display.width,display.height,display.u,display.v,display.u2,display.v2);
		}else{
	...
	public void checkClick(int mouseX,int mouseY) {
		if(!hover&&isMouseInBox(mouseX, mouseY, x, y, x+buttonDimensions.width, y+buttonDimensions.height)&&Mouse.isButtonDown(0))return;
		if(isMouseInBox(mouseX, mouseY, x, y, x+buttonDimensions.width, y+buttonDimensions.height))hover=true;
		else hover=false;
