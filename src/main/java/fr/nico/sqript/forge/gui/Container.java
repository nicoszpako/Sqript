...
public class Container extends Widget {
	...
	protected ResourceLocation imageSource;
	...
	int bestHeight = 0;
	public boolean takeParent = true;
	...
	public void draw(int mouseX, int mouseY) {
		...
		if (this.getParent() != null && takeParent) {
		...
		GL11.glEnable(GL11.GL_SCISSOR_TEST);
		{
			...
			if (style.drawBackground) {
				...
				if (draw_type == DRAW_IMAGE_FIXED) {
					Minecraft.getMinecraft().getTextureManager().bindTexture(imageSource);
					drawTexturedRect((double) this.x, (double) this.y, (int) display.width, (int) display.height);
				}
			}
			...
			for (int i = 0; i < content.size(); i++) {
				...
				if (layout == EnumLayout.FLOW) {
					...
					if (!w.isRelativeFixed) {
						if (w.style.getHeight() + w.style.margin_top + 2 * w.style.bordersize > bestHeight) {
							bestHeight = i > 0
									? content.get(i - 1).style.getHeight() + content.get(i - 1).style.margin_top
											+ 2 * content.get(i - 1).style.bordersize
									: w.style.getHeight() + w.style.margin_top + 2 * w.style.bordersize;
						}
						...
						w.setY(this.y + mtop + w.style.margin_top + w.style.bordersize + this.style.padding_top);
					...
					if (!w.isRelativeFixed) {
						...
						w.y = this.y + w.style.bordersize + w.style.margin_top + mtop + this.style.padding_top;
