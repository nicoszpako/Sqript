...
public class TextField extends Widget implements IClickListener, IKeyListener {
    ...
    protected int maxStringLength = 1024;
    ...
    public boolean isNull() {
        return getText().isEmpty() || getText().replaceAll(" ", "").isEmpty();
    }
    ...
    public void writeText(String textToWrite) {
        if (onlyNumbers) {
            if (!integerOnly && textToWrite.equals(".") && this.text.contains(".")) return;
            else if (!(NumberUtils.isCreatable(textToWrite)) && !textToWrite.equals(".")) return;
    ...
    /**
     * Notifies this text box's {@linkplain GuiPageButtonList.GuiResponder
    ...
    public void mouseClick(int mouseX, int mouseY, int mouseButton) {
        boolean flag = mouseX >= this.x && mouseX < this.x + this.style.width && mouseY >= this.y
                && mouseY < this.y + this.style.height;
    ...
    public void draw(int a, int b) {
        ...
        if (flag1) {
            ...
            this.fontRenderer.drawString("\2477_", (float) k1, (float) i1, i, false);
    ...
    protected void drawSelectionBox(int startX, int startY, int endX, int endY) {
        ...
        if (endX > this.x + this.style.width) {
            endX = this.x + this.style.width;
        }

        if (startX > this.x + this.style.width) {
            startX = this.x + this.style.width;
        }
